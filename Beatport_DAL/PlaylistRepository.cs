    ...
    public PlaylistDto? GetPlaylist(int id)
{
    using (MySqlConnection connection = new MySqlConnection(connectionStr))
    {
        ...
        // Query to get the playlist with its linked songs
        string query = $@"
            SELECT 
                p.Id AS PlaylistId,
                ...
                while (reader.Read())
                {
                    ...
                    if (playlistDto == null)
                    {
                        playlistDto = new PlaylistWithSongsDto()
                        {
                            ...
                            Description = reader.IsDBNull(reader.GetOrdinal("PlaylistDescription")) ? null : reader.GetString("PlaylistDescription"),
